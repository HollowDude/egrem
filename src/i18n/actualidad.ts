import type { Lang } from './index';

export const actualidad: Record<Lang, Record<string, string>> = {
  es: {
    'actualidad.page_title': 'Actualidad | EGREM',
    'actualidad.page_description':
      'Mantente al día con las últimas noticias, artículos, blogs y boletines de EGREM, la casa de la música cubana.',
    'actualidad.hero.title': 'Actualidad',
    'actualidad.hero.subtitle': 'Noticias, artículos y novedades de la casa de la música cubana.',
    'actualidad.filter.all': 'Todo',
    'actualidad.filter.noticia': 'Noticias',
    'actualidad.filter.article': 'Artículos',
    'actualidad.filter.blog': 'Blog',
    'actualidad.filter.boletin_archivo': 'Boletines',
    'actualidad.filter.patrimonio': 'Patrimonio',
    'actualidad.badge.noticia': 'Noticia',
    'actualidad.badge.article': 'Artículo',
    'actualidad.badge.blog': 'Blog',
    'actualidad.badge.boletin_archivo': 'Boletín',
    'actualidad.badge.patrimonio': 'Patrimonio',
    'actualidad.read_more': 'Leer más',
    'actualidad.no_results': 'No encontramos resultados para esta categoría.',
    'actualidad.section_empty': 'No hay contenido disponible en esta sección.',
    'actualidad.loading': 'Cargando...',
    'actualidad.error': 'No pudimos cargar el contenido. Inténtalo de nuevo más tarde.',
    'actualidad.by_author': 'Por {author}',
    'actualidad.detail.related_title': 'Contenido Relacionado',
    'actualidad.detail.share': 'Compartir',
    'actualidad.detail.copy_link': 'Copiar enlace',
    'actualidad.detail.link_copied': '¡Enlace copiado!',
    'actualidad.detail.share_whatsapp': 'Compartir en WhatsApp',
    'actualidad.detail.share_instagram': 'Compartir en Instagram',
    'actualidad.detail.share_facebook': 'Compartir en Facebook',
    'actualidad.detail.back': 'Volver a Actualidad',
    'actualidad.detail.not_found_title': 'Contenido no encontrado',
    'actualidad.detail.reading_time': '{min} min de lectura',
    'actualidad.detail.related_artists_title': 'Artistas Relacionados',
    'actualidad.detail.related_events_title': 'Eventos Próximos',
    'actualidad.detail.view_profile': 'Ver Perfil Completo',
    'actualidad.detail.comments_title': 'Comentarios',
    'actualidad.detail.comments_placeholder': 'Añade tu comentario...',
    'actualidad.detail.comments_submit': 'Publicar',
    'actualidad.detail.comments_login_required': 'Inicia sesión para comentar',
    'actualidad.detail.comments_reply': 'Responder',
    'actualidad.detail.comments_confirmation':
      'Comentario recibido. Está pendiente de aprobación y se publicará cuando un moderador lo apruebe.',
    'actualidad.detail.newsletter_title': 'Suscríbete a este blog',
    'actualidad.detail.newsletter_subtitle': 'Recibe en tu correo las novedades de este blog.',
    'actualidad.detail.newsletter_email_label': 'Tu correo',
    'actualidad.detail.newsletter_subscribe': 'Suscribirme a este blog',
    'actualidad.detail.newsletter_unsubscribe': 'Darme de baja de este blog',
    'actualidad.detail.newsletter_login_required': 'Inicia sesión para suscribirte a este blog',
  },
  en: {
    'actualidad.page_title': 'News | EGREM',
    'actualidad.page_description':
      'Stay up to date with the latest news, articles, blogs and newsletters from EGREM, the home of Cuban music.',
    'actualidad.hero.title': 'News',
    'actualidad.hero.subtitle': 'News, articles and updates from the home of Cuban music.',
    'actualidad.filter.all': 'All',
    'actualidad.filter.noticia': 'News',
    'actualidad.filter.article': 'Articles',
    'actualidad.filter.blog': 'Blog',
    'actualidad.filter.boletin_archivo': 'Newsletters',
    'actualidad.filter.patrimonio': 'Heritage',
    'actualidad.badge.noticia': 'News',
    'actualidad.badge.article': 'Article',
    'actualidad.badge.blog': 'Blog',
    'actualidad.badge.boletin_archivo': 'Newsletter',
    'actualidad.badge.patrimonio': 'Heritage',
    'actualidad.read_more': 'Read more',
    'actualidad.no_results': 'No results found for this category.',
    'actualidad.section_empty': 'No content available in this section.',
    'actualidad.loading': 'Loading...',
    'actualidad.error': 'Could not load content. Please try again later.',
    'actualidad.by_author': 'By {author}',
    'actualidad.detail.related_title': 'Related Content',
    'actualidad.detail.share': 'Share',
    'actualidad.detail.copy_link': 'Copy link',
    'actualidad.detail.link_copied': 'Link copied!',
    'actualidad.detail.share_whatsapp': 'Share on WhatsApp',
    'actualidad.detail.share_instagram': 'Share on Instagram',
    'actualidad.detail.share_facebook': 'Share on Facebook',
    'actualidad.detail.back': 'Back to News',
    'actualidad.detail.not_found_title': 'Content not found',
    'actualidad.detail.reading_time': '{min} min read',
    'actualidad.detail.related_artists_title': 'Related Artists',
    'actualidad.detail.related_events_title': 'Upcoming Events',
    'actualidad.detail.view_profile': 'View Full Profile',
    'actualidad.detail.comments_title': 'Comments',
    'actualidad.detail.comments_placeholder': 'Add your comment...',
    'actualidad.detail.comments_submit': 'Publish',
    'actualidad.detail.comments_login_required': 'Log in to comment',
    'actualidad.detail.comments_reply': 'Reply',
    'actualidad.detail.comments_confirmation':
      'Comment received. It is pending approval and will be published once a moderator approves it.',
    'actualidad.detail.newsletter_title': 'Subscribe to this blog',
    'actualidad.detail.newsletter_subtitle': 'Get this blog news delivered to your inbox.',
    'actualidad.detail.newsletter_email_label': 'Your email',
    'actualidad.detail.newsletter_subscribe': 'Subscribe to this blog',
    'actualidad.detail.newsletter_unsubscribe': 'Unsubscribe from this blog',
    'actualidad.detail.newsletter_login_required': 'Log in to subscribe to this blog',
  },
};
