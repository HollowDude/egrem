...
function parseAddress(raw: unknown): NhSedeAddress | null {
...
function parsePhoneList(raw: unknown): NhSedePhone[] {
...
function parseHorario(raw: unknown): { value: string; end_value: string } | null {
  if (!raw || typeof raw !== 'object') return null;
  const r = raw as Record<string, unknown>;
